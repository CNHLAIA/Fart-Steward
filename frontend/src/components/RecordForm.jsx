...
const DURATIONS = [
  { value: 'very_short', label: 'Very Short (<1s)' },
...
const SMELL_LEVELS = [
  ...
  { value: 'extremely_stinky', label: 'Toxic' }
];
...
export default function RecordForm() {
  ...
  if (loading && !types.length) return <div className="p-4">Loading...</div>;

  return (
    <div className="max-w-md mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      <div className="px-6 py-4 bg-purple-600 text-white">
        <h2 className="text-xl font-bold">{id ? 'Edit Fart' : 'New Fart'}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        ...
        <div>
          <label htmlFor="timestamp" className="block text-sm font-medium text-gray-700 mb-1">Date & Time</label>
          <input
            id="timestamp"
            type="datetime-local"
            required
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="type-select">Type</label>
          {isCustomType ? (
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Smell Level</label>
          <div className="grid grid-cols-2 gap-2">
            {SMELL_LEVELS.map((opt) => (
              <button
                key={opt.value}
                type="button"
                onClick={() => setFormData({...formData, smell_level: opt.value})}
                className={`px-3 py-2 text-sm rounded-md border ${
                  formData.smell_level === opt.value
              ...
              <div className="flex rounded-md shadow-sm" role="group">
                {TEMPERATURES.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setFormData({...formData, temperature: opt.value})}
              ...
              <div className="flex rounded-md shadow-sm" role="group">
                {MOISTURES.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setFormData({...formData, moisture: opt.value})}
                    className={`flex-1 px-3 py-2 text-sm border first:rounded-l-md last:rounded-r-md ${
                      formData.moisture === opt.value
